...
- 为AgentState全部写入建立串行队列
  - [done] 确认`completeToolCall`与标题写入在`ExternalWrite` CAS准入处竞争
  - [done] 固化全部写入共用队列、状态校验时机与标题patch不变量
  - 为每个AgentState实例加入覆盖全部写入的公平Mutex
  - 将`requestResponseApi()`与compaction纳入同一写入临界区，移除其CAS竞争准入
  - 让标题写入在队列内基于最新settings进行条件`threadName` patch
  - 覆盖请求、compaction、工具完成、标题写入、settings/plan保留、取消等待者与非法状态的串行测试
  - 运行受影响的AgentState和session-title Gradle测试

# Details

- 状态：计划已完成，尚未开始实现；后续节点不构成自动实施授权。
