...
- `auth.yml` 位于 Kodex 私有数据目录，与 `settings.yml` 并列。
- `settings.yml.auth_source` 是认证来源的唯一选择：默认`codex`只读当前 Codex Home 的`auth.json`，`kodex`使用私有`auth.yml`。
- `auth.yml`只保存 Kodex 管理的凭据和续期元数据，不再保存来源类型；Kodex 不会写入 Codex 的`auth.json`。
- 凭据本身不进入`KodexGlobalSettings`；仅认证来源作为全局设置持久化。
