...
- [done] 规划Codex Session导入兼容性
  - [done] 盘点Codex thread、rollout、subagent关系和Kodex AgentStorage的现有映射
  ...
  - [done] 定义完整AgentStorage树到新Kodex Session的staging与原子发布语义
  - [done] 定义全局设置入口的根thread选择、导入进度和结果反馈
  - [done] 协调现有KodexSession重构与Multi-agent V2
...
- 每个Codex thread独立转换为一个AgentStorage；subagent关系由KodexSession树表达，不将subagent历史合并进root AgentStorage。
