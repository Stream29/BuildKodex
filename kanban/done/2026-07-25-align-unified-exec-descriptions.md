...
- Rust与Kotlin均在Windows上为`exec_command`追加专门的安全提示。
- Kotlin的`shell`字段使用与Rust一致的固定通用描述。
- 保留`tty`模型参数，并验证pipe与真实PTY实现。
- Windows Native已在`win11` VM通过完整`mingwX64Test`。
- Windows初始`exec_command`的`yield_time_ms`有效下限为10000毫秒，与上游描述和行为一致。
