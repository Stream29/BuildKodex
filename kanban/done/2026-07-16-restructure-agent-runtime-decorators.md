...
- [done] 重构 agent runtime 为可组合的工具装饰器
  - [done] 将 CodexAgentLoopImpl 改名为 CodexAgentCompactionRuntime
  - [done] 提供 CodexAgentState.compactionRuntime() 扩展
...
每个工具装饰器只处理自己负责的工具调用，并通过 `CodexAgentRuntime` 的 Kotlin 委托保留底层原子状态和内层 `resume()` 行为。
