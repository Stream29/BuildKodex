...
- 回退范围限于`Kodex/agent-storage/in-memory/src/commonMain/kotlin/io/github/stream29/kodex/agentstorage/inmemory/InMemoryKodexAgentStorage.kt`和`Kodex/agent-storage/in-memory/build.gradle.kts`。测试只在必要时调整。
- 不回退repository接入、`KodexAgentStorageHandle`、`release()`或操作级补偿。
