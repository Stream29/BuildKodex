...
- [done] 重新设计Tool Runtime与AgentSettings边界
  - [done] 追溯新增`CodexAgentRuntime.resume(tools: List<ToolSpec>)`及相关修改的动机
  ...
  - [done] 收敛实现与测试
    ...
    - [done] 在请求前将generation原子同步到`CodexAgentSettings.tools`
...
- 用户已授权按任务顺序实施。
- `CodexAgentSettings.tools`是Responses请求工具定义的唯一事实来源，不允许request-local参数覆盖。
