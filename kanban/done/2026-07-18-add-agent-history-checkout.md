...
- [done] 实现Agent历史checkout语义
  - [done] 为`MutableCodexAgentStorage`定义跨全部timeline的原子后缀回退原语
  - [done] 在内存存储中实现该原语并保留索引`0`的初始化快照
  - [done] 在`CodexAgentState`中暴露语义化的checkout操作
...
exclusive boundary保持一致。TUI不能直接修改storage，否则`CodexAgentState`
缓存的`StateFlow`会失效。
