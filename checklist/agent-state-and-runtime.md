...
- Remote compaction v2安装checkpoint时，明文prefix只保留`UserMessage`与完成的`StablePlanUpdate`；PlanUpdate按完整function call/output组识别，两类保留项按原顺序共享64,000-token窗口并从最新向前选择，边界UserMessage可以截断，PlanUpdate不可拆分；provider返回的encrypted compaction item继续独立保留。
