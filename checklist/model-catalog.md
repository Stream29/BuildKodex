...
- 普通Responses只根据OpenAI实际报告且可解读的`usage.total_tokens`推进计数；缺失值不新增change point，也不做本地估算。每个成功compaction checkpoint例外地写入synthetic `0`，预算消费者直接使用该值，直到后续普通response报告新计数。结构化快照及尽力采集规则见[Token-count Timeline](token-count-timeline.md)。
