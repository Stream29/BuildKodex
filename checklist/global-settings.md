...
- `cli:settings`只承载跨会话的应用设置，不得混入按会话版本化的`KodexAgentSettings`。
...
- `KodexGlobalSettings.newSession`是新Session默认配置的唯一持久化真源；`Settings > New session`无论当前标签为何，都必须显示和更新这份全局默认值。虚拟`NewSession`可保留其创建时的内存草稿，但该草稿不得成为设置页的读写目标。
