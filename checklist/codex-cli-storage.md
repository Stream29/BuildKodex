...
- 将Codex CLI本地设置和Session兼容逻辑收敛在`CodexLite/openai/codex-cli-storage`。
...
- 在`$CODEX_HOME/GlobalSettings.yml`中保存Codex Lite自有的稀疏全局设置override；缺失字段继续继承Codex值。
...
- 导入用户选择的根thread及其全部可达subagent thread，并根据Codex parent/path信息重建CodexSession树。
