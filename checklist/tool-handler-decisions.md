...
- Put state-bound handlers such as `update_plan` and Multi-agent tools in their corresponding tool module as `CodexAgentState` factories.
...
- Use CodexToolRuntime for normal local tools. It borrows a fixed `List<Tool>`, dynamic `StateFlow<List<Tool>>`, and `StateFlow<ToolSearchEngine>` supplied by composition.
- Keep tool construction, resource ownership, and catalog rebuilding outside CodexToolRuntime.
