...
- 每个 persisted Session ViewModel 直接包含唯一的常驻 `rootAgent`；当前不存在 Agent topology、selected child 或 Agent ViewModel registry。
- 使用 root `sessionIndex` 标识 Session ViewModel；root Agent ViewModel 由其稳定对象身份在应用内识别。
- Root Agent ViewModel 在创建 Session ViewModel 时立即 materialize；当前不按需 materialize 子 Agent。
...
- 将终端布局、焦点、hover 和 popup anchor 保留为 renderer-local 状态；History 的
...
- Opened Session registry 归 Application；catalog 列表归独立 `SessionCatalogViewModel`，该 child 只在 Application 打开
...
- Persisted Session 直接发布稳定 `rootAgent`；当前没有 Agent selection 命令。
- Root Agent 持有稳定 composer、history、request-user-input 与 Shell registry child handle；Session state 不复制这些 child
  的 mutable state。
...
- 每个 persisted Session ViewModel 必须从自身 lifecycle scope 创建 root `KodexSessionRepository` 子级；root Session、runtime
  与 root lease 均为其后代。切换 tab 不释放这些资源；Session close 或 shutdown 才关闭 repository 并释放 lease。
...
- Session ViewModel 关闭 root Agent handle 后才释放所属 Session manager 资源；Agent runtime 与 storage 的底层释放仍由
  Session manager 负责。

## Per-agent ViewModel

- 每个 Agent ViewModel 只发布一个 Agent 的完整 `StateFlow<KodexAgentSettings>`、execution、token count、history action、
  notification 与 lifecycle；committed、pending tool 和 streaming History 状态由 history child 统一拥有和分别发布。
- `threadName` 与 `plan` 从 `KodexAgentSettings` 读取；不得另建 Agent summary 或 plan state。
- Agent settings 通过 model、working directory、reasoning effort、service tier 与 request-user-input mode 的按字段方法更新；每个方法必须基于该
...
- Agent ViewModel 的事件和异步 completion 必须捕获显式 Agent ViewModel 对象身份与 revision，不能在执行时重新解析应用级 active
  Session。
- Session/Agent ViewModel 不随 selected tab 的变化而销毁；切换只替换当前 renderer 的显示对象。
...
- Persisted Session 只 materialize 一个 root Agent ViewModel；Session ViewModel 不维护 Agent Tree、递归拓扑或后台 Agent registry。
- Materialize root Agent ViewModel 时绑定既有 root AgentSession storage，只读取轻量状态与有限 history tail，并合并执行层仍在进行的
  stream/activity；不得构造一次完整 history 快照。
- Session manager 操作必须显式接收 Session identity 或 root Agent 的稳定 handle；不得依赖隐式 selected routing。
...
- Open Session 创建或复用 Session ViewModel，并且只 materialize root Agent ViewModel。
- 切换 selected Session 只更新应用级下标；每个 Session ViewModel 保留自己的 root Agent 和 Agent UI drafts。
