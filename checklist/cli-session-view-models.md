...
- 保留一个应用级 `CodexCliAppViewModel`，并为每个已打开的真实 Session 建立一个稳定的 `KodexSessionViewModel`。
...
- `KodexGlobalSettings.newSession` 继续是 defaults 的唯一持久化真源；NewSession ViewModel 不保存第二份持久化 authority。
